...
def xss_run_scan(url, payload_file_path, blind_mode):
    ...
    try:
        ...
        if scan_states['xss']['should_stop']:
            ...
            if not scan_states['xss']['vulnerability_found']:
                scan_states['xss']['results'].append("[INFO] No vulnerabilities found")
    except Exception as e:
        ...
        scan_states['xss']['should_stop'] = False
        cleanup_scan_state('xss')  # Add this line
...
def traversal_run_scan(base_url, payload_file):
    try:
        ...
        if scan_states['traversal']['stop_flag']:
             ...
             scan_states['traversal']['queue'].put("[INFO] Scan completed")
             if not scan_states['traversal']['vulnerability_found']:
                 scan_states['traversal']['queue'].put("[INFO] No vulnerabilities found")
        
    except Exception as e:
        scan_states['traversal']['queue'].put(f"[ERROR] Scan failed: {str(e)}")
    finally:
        scan_states['traversal']['running'] = False
        cleanup_scan_state('traversal')  # Add this line
        safe_remove(payload_file)
    ...
    def event_stream():
        while True:
            try:
                ...
                time.sleep(0.5)
        if not scan_states['sql']['vulnerability_found']:
             yield "data: [INFO] No vulnerabilities found\n\n"
...
# In the stop_traversal_scan route
...
def get_saved_url_files():
    """Get list of saved URL files"""
    try:
        files = []
        for filename in os.listdir('.'):
            if filename.startswith('extracted_urls') or filename.startswith('subdomains_'):
...
@app.route('/reset_all_scans', methods=['POST'])
def reset_all_scans():
    for scan_type in scan_states:
        cleanup_scan_state(scan_type)
    return jsonify({'status': 'all scans reset'})
